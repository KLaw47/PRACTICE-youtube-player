...
const eventListeners = () => {
  ...
  document.querySelector('#filterContainer').addEventListener('click', (e) => {
    ...
    if (e.target.id === "clear") {
      ...
      const favs = data.filter(vids => vids.favorite === true);
      cardsOnDom(favs);
    } else if (e.target.id) {
      const topic = data.filter(vids => vids.category === e.target.id);
      cardsOnDom(topic);
    }
  });
  ...
  document.querySelector('#cardContainer').addEventListener('click', (e) => {
    ...
    if (e.target.id) {
      ...
      //const videoStuff = e.target.id.spilt("--");

      const [method, videoId] = e.target.id.split("--"); //mostly what you see in practice. destructuring
